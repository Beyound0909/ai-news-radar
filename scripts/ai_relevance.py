...
FINANCE_RELEVANCE_THRESHOLD = 0.65
...
LABEL_KEYWORDS = [
    ...
    ("finance_markets", ["stock", "equity", "bond", "futures", "etf", "nasdaq", "s&p", "dow", "A股", "港股", "美股", "指数", "基金", "期货", "外汇", "央行", "利率", "降息", "加息"]),
    ("crypto_web3", ["bitcoin", "ethereum", "crypto", "blockchain", "defi", "nft", "比特币", "以太坊", "加密货币", "区块链", "代币"]),
    ("macro_economy", ["fed", "fomc", "cpi", "gdp", "pmi", "inflation", "treasury", "yield", "通胀", "通缩", "GDP", "PMI", "CPI", "财政", "货币"]),
]
...
def _finance_label_for_text(text: str) -> str:
    for label, keywords in LABEL_KEYWORDS:
        if label.startswith("finance") or label.startswith("crypto") or label.startswith("macro"):
            if contains_any_keyword(text, keywords):
                return label
...
def score_ai_relevance(record: dict[str, Any]) -> dict[str, Any]:
    ...
    finance_signals = matched_keywords(text, FINANCE_KEYWORDS)
    finance_noise = matched_keywords(text, FINANCE_NOISE_KEYWORDS)
    has_finance = bool(finance_signals)
    is_finance_source = any(d in url_host for d in FINANCE_DOMAINS) or (site_id.startswith("opmlrss") and has_finance)

    if is_finance_source:
        if finance_noise and not has_finance and not has_ai:
            return _result(
                is_ai_related=False,
                score=0.15,
                label="finance_spam",
                reason="finance_noise_from_trusted_source",
                signals=finance_signals,
                noise=finance_noise,
            )
        if has_finance:
            fin_score = FINANCE_RELEVANCE_THRESHOLD + min(0.2, 0.04 * len(finance_signals))
            if has_ai:
                fin_score = max(fin_score, AI_RELEVANCE_THRESHOLD)
                return _result(
                    is_ai_related=True,
                    score=fin_score,
                    label=_label_for_text(text, bool(tech_signals)) if has_ai else "finance_markets",
                    reason="finance_source_with_ai_signal",
                    signals=ai_signals + finance_signals + tech_signals,
                    noise=noise + finance_noise,
                )
            return _result(
                is_ai_related=True,
                score=fin_score,
                label=_finance_label_for_text(text),
                reason="finance_source_matched_keywords",
                signals=finance_signals + tech_signals,
                noise=noise + finance_noise,
            )
        # Finance source but no finance keywords - still keep with moderate score
        return _result(
            is_ai_related=True,
            score=FINANCE_RELEVANCE_THRESHOLD,
            label="finance_general",
            reason="finance_source_default_keep",
            signals=finance_signals + tech_signals,
            noise=noise + finance_noise,
        )

    # Non-finance, non-AI source but has finance content
    if has_finance and not has_ai and not has_broad_ai:
        fin_score = 0.3 + min(0.2, 0.04 * len(finance_signals))
        if finance_noise:
            fin_score -= min(0.15, 0.05 * len(finance_noise))
        return _result(
            is_ai_related=fin_score >= FINANCE_RELEVANCE_THRESHOLD,
            score=fin_score,
            label=_finance_label_for_text(text) if fin_score >= FINANCE_RELEVANCE_THRESHOLD else "not_ai",
            reason="finance_signal_from_general_source",
            signals=finance_signals,
            noise=noise + finance_noise,
        )
